...
namespace GunslingerGame
{
	...
	/// </summary>
	public class GSGGlobalMusic:MonoBehaviour 
	{
		...
		void  Awake()
		{
			...
			//Keep only music objects older than 0 seconds
			if ( musicObjects.Length > 1 )
			{
				foreach( var musicObject in musicObjects )
				{
					if ( musicObject.GetComponent<GSGGlobalMusic>().instanceTime <= 0 )    Destroy(gameObject);
				}
			}
		}
